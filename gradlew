...
if [ "$cygwin" = "true" ] || [ "$msys" = "true" ] ; then
    ...
    # Now convert the arguments - kludge to limit to approximate to maxline
    # on NT/Windows box we run with maximum file descriptor limit
    MAX_FD_LIMIT=`ulimit -H -n`
    if [ $? -eq 0 ] ; then
        if [ "$MAX_FD" = "maximum" ] || [ "$MAX_FD" = "max" ] ; then
            MAX_FD="$MAX_FD_LIMIT"
        ...
        warn "Could not query maximum file descriptor limit; using 2048"
    fi
fi
